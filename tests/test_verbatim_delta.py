import pickle
from decimal import Decimal

import pytest

from whenever import VerbatimDelta

from .common import AlwaysEqual, NeverEqual
from .test_date_delta import INVALID_DDELTAS, VALID_DDELTAS
from .test_time_delta import INVALID_TDELTAS, VALID_TDELTAS

UNITS = "years months weeks days hours minutes seconds".split()


class TestInit:

    @pytest.mark.parametrize(
        "kwargs, expect_sign",
        [
            ({"days": 5}, 1),
            ({"weeks": 1}, 1),
            ({"years": 2}, 1),
            ({"years": 0}, 0),
            ({"minutes": 90}, 1),
            ({"days": 5, "weeks": 1, "years": 2, "minutes": 90}, 1),
            ({"minutes": -80, "weeks": -1}, -1),
            ({"days": 0, "minutes": -1}, -1),
            ({"days": -1, "minutes": 0}, -1),
            ({"days": 3, "seconds": 9}, 1),
            ({"seconds": 0}, 0),
            ({"seconds": -30}, -1),
            # TODO: test inputting fractional seconds
            ({"days": 3, "seconds": 1}, 1),
            ({"days": 3, "nanoseconds": 500_000_00}, 1),
            ({"nanoseconds": 50}, 1),
            ({"seconds": 50, "nanoseconds": 45_000}, 1),
        ],
    )
    def test_simple_valid(self, kwargs, expect_sign):
        d = VerbatimDelta(**kwargs)
        assert d.sign == expect_sign
        for unit in UNITS:
            assert getattr(d, unit) == kwargs.get(unit, 0)

    def test_no_components(self):
        with pytest.raises(ValueError, match="At least one"):
            VerbatimDelta()

    @pytest.mark.parametrize(
        "kwargs",
        [
            {"days": 5, "seconds": -10},
            {"years": -2, "minutes": 30},
            {"seconds": 3, "nanoseconds": -3},
        ],
    )
    def test_mixed_sign(self, kwargs):
        with pytest.raises(ValueError, match="sign"):
            VerbatimDelta(**kwargs)

    @pytest.mark.parametrize(
        "value, unit",
        [
            (10_000, "years"),
            (-10_000, "years"),
            (10_000 * 12, "months"),
            (-10_000 * 12, "months"),
            (9_999 * 54, "weeks"),
            (-9_999 * 54, "weeks"),
            (10_000 * 366, "days"),
            (-10_000 * 366, "days"),
            (10_000 * 366 * 24, "hours"),
            (-10_000 * 366 * 24, "hours"),
            (10_000 * 366 * 24 * 60, "minutes"),
            (-10_000 * 366 * 24 * 60, "minutes"),
            (10_000 * 366 * 24 * 60 * 60, "seconds"),
            (-10_000 * 366 * 24 * 60 * 60, "seconds"),
            (1_000_000_000, "nanoseconds"),
            (-1_000_000_000, "nanoseconds"),
        ],
    )
    def test_range(self, value, unit):
        kwargs = {unit: value}
        with pytest.raises((ValueError, OverflowError), match="range|inf|NaN"):
            VerbatimDelta(**kwargs)

    def test_float_seconds(self):

        d = VerbatimDelta(seconds=9_000, nanoseconds=1)
        assert d.float_seconds == 9_000.000000001


@pytest.mark.parametrize(
    "d, expected",
    [
        (VerbatimDelta(days=5), (5,)),
        (VerbatimDelta(weeks=1, years=2, minutes=8), (2, 1, 8)),
        (VerbatimDelta(weeks=-1, minutes=0), (-1, 0)),
        (
            VerbatimDelta(years=1, seconds=9_000, nanoseconds=20),
            (1, 9_000, 20),
        ),
    ],
)
def test_values(d, expected):
    assert d.values() == expected


@pytest.mark.parametrize(
    "d, expected",
    [
        (VerbatimDelta(days=5), {"days": 5}),
        (
            VerbatimDelta(weeks=1, years=2, minutes=8),
            {"years": 2, "weeks": 1, "minutes": 8},
        ),
        (
            VerbatimDelta(weeks=-1, minutes=-80),
            {"weeks": -1, "minutes": -80},
        ),
        (
            VerbatimDelta(years=1, seconds=9_000_000_000, nanoseconds=1),
            {"years": 1, "seconds": 9_000_000_000, "nanoseconds": 1},
        ),
    ],
)
def test_dict(d, expected):
    assert d.dict() == expected
    # ensure the order is correct
    assert list(d.dict()) == list(expected)


class TestEq:
    def test_notimplemented(self):
        d = VerbatimDelta(days=5)
        assert d != NeverEqual()
        assert NeverEqual() != d
        assert not d == NeverEqual()
        assert not NeverEqual() == d

        assert d == AlwaysEqual()
        assert AlwaysEqual() == d
        assert not d != AlwaysEqual()
        assert not AlwaysEqual() != d

        assert d != 5  # type: ignore[comparison-overlap]
        assert 5 != d  # type: ignore[comparison-overlap]

    def test_equal(self):
        d1 = VerbatimDelta(days=5, years=2)
        d2 = VerbatimDelta(days=5, years=2)
        d3 = VerbatimDelta(days=5, years=3)
        assert d1 == d2
        assert not d1 != d2
        assert d1 != d3
        assert not d1 == d3

    def test_zero_is_not_same_as_missing(self):
        d1 = VerbatimDelta(weeks=1)
        d2 = VerbatimDelta(weeks=1, seconds=0)
        assert d1 != d2
        assert not d1 == d2


class TestFormatIso:

    @pytest.mark.parametrize(
        "d, expected",
        [
            (VerbatimDelta(seconds=0), "PT0S"),
            (VerbatimDelta(days=0), "P0D"),
            (VerbatimDelta(days=5), "P5D"),
            (VerbatimDelta(days=5, seconds=0), "P5DT0S"),
            (
                VerbatimDelta(
                    years=3, months=6, days=4, hours=12, minutes=30, seconds=5
                ),
                "P3Y6M4DT12H30M5S",
            ),
            (VerbatimDelta(days=23, hours=23), "P23DT23H"),
            (VerbatimDelta(years=4), "P4Y"),
            (VerbatimDelta(seconds=0), "PT0S"),
            (VerbatimDelta(months=-6), "-P6M"),
            (VerbatimDelta(minutes=-600), "-PT600M"),
        ],
    )
    def test_format_iso(self, d, expected):
        assert d.format_iso() == expected

    def test_lowercase_units(self):
        d = VerbatimDelta(
            years=1, months=2, days=3, hours=4, minutes=5, seconds=6
        )
        assert d.format_iso(lowercase_units=True) == "P1y2m3dT4h5m6s"
        assert (
            VerbatimDelta(seconds=0).format_iso(lowercase_units=True) == "PT0s"
        )


def test_repr():
    d = VerbatimDelta(
        years=3,
        months=6,
        days=4,
        hours=12,
        minutes=30,
        seconds=5,
        nanoseconds=400_000_000,
    )
    assert repr(d) == 'VerbatimDelta("P3y6m4dT12h30m5.4s")'
    assert repr(VerbatimDelta(seconds=0)) == 'VerbatimDelta("PT0s")'
    assert (
        repr(VerbatimDelta(hours=-1, minutes=0)) == 'VerbatimDelta("-PT1h0m")'
    )


INVALID_DELTAS = [
    "P",
    "PT0.0000000001S",  # too many decimal places
    "",
    "3D",
    "-PT",
    "PT",
    "+PT",
    "P1YX3M",  # invalid separator
    "PT𝟙H",  # non-ascii
    # incomplete
    "P3DT",
    "P3DT4",
    "P3DT4h0",
    "P3D4",
    "P3D4T",
    # too many digits
    "P9999999999999999999S",
    # out of range
    "P14000Y",
    "P180000M",
    "PT180000000H",
    # unit mixups
    "P3DT4HM",
    "P3DT4H8X",
    "P3DT4M3H",
    *INVALID_DDELTAS,
    *INVALID_TDELTAS,
]
# some formats are invalid as date or time deltas alone, but valid as combined
INVALID_DELTAS.remove("PT3M")
INVALID_DELTAS.remove("P1Y2M3W4DT1H2M3S")
INVALID_DELTAS.remove("P1YT0S")
INVALID_DELTAS.remove("P1D")
INVALID_DELTAS.remove("P1YT4M")
INVALID_DELTAS.remove("PT4M3H")


class TestParseIso:

    @pytest.mark.parametrize(
        "s, expected",
        [
            (
                "P3Y6M4DT12H30M5S",
                VerbatimDelta(
                    years=3, months=6, days=4, hours=12, minutes=30, seconds=5
                ),
            ),
            ("P23dt23h", VerbatimDelta(days=23, hours=23)),
            ("P4Y", VerbatimDelta(years=4)),
            ("PT0s", VerbatimDelta(seconds=0)),
            ("-P6M", VerbatimDelta(months=-6)),
            (
                "PT1H2M3.000004S",
                VerbatimDelta(
                    hours=1, minutes=2, seconds=3, nanoseconds=4_000
                ),
            ),
            (
                "-PT58m200.999996S",
                VerbatimDelta(
                    minutes=-58, seconds=-200, nanoseconds=-999_996_000
                ),
            ),
            ("PT0S", VerbatimDelta(seconds=0)),
            ("PT00000.000000001S", VerbatimDelta(seconds=0, nanoseconds=1)),
            ("PT00000.000000000S", VerbatimDelta(seconds=0, nanoseconds=0)),
            ("PT450.000000001S", VerbatimDelta(seconds=450, nanoseconds=1)),
            ("-PT0.000001S", VerbatimDelta(nanoseconds=-1_000)),
            ("PT1.999997S", VerbatimDelta(seconds=1, nanoseconds=999_997_000)),
            # ("PT5H", TimeDelta(hours=5)),
            # ("PT400H", TimeDelta(hours=400)),
            # ("PT400H0M0.0S", TimeDelta(hours=400)),
            # ("-PT4M", TimeDelta(minutes=-4)),
            # ("PT0S", TimeDelta()),
            # ("PT3M", TimeDelta(minutes=3)),
            # ("+PT3M", TimeDelta(minutes=3)),
            # ("PT0M", TimeDelta()),
            # ("PT0.000000000S", TimeDelta()),
            # # extremely long but still valid
            # (
            #     "PT0H0M000000000000000300000000000.000000000S",
            #     TimeDelta(seconds=300_000_000_000),
            # ),
            # ("PT316192377600S", TimeDelta.MAX),
            # # non-uppercase
            # (
            #     "pt58m2.999996s",
            #     TimeDelta(hours=1, minutes=-2, seconds=3, microseconds=-4),
            # ),
            # ("PT316192377600s", TimeDelta.MAX),
            # ("PT400h", TimeDelta(hours=400)),
            # # comma instead of dot
            # ("PT1,999997S", TimeDelta(seconds=2, microseconds=-3)),
        ],
    )
    def test_valid(self, s: str, expected: VerbatimDelta):
        assert VerbatimDelta.parse_iso(s) == expected

    # TODO:
    # @pytest.mark.parametrize("input, expect", VALID_DDELTAS)
    # def test_date_only(self, input, expect):
    #     assert VerbatimDelta.parse_iso(input) == (expect + TimeDelta())

    @pytest.mark.parametrize("s", INVALID_DELTAS)
    def test_invalid(self, s: str):
        with pytest.raises(ValueError):
            VerbatimDelta.parse_iso(s)


@pytest.mark.parametrize(
    "d",
    [
        VerbatimDelta(
            years=1,
            months=2,
            weeks=3,
            days=4,
            hours=5,
            minutes=6,
            seconds=7,
            nanoseconds=8,
        ),
        VerbatimDelta(days=5),
        VerbatimDelta(days=-5, hours=-3),
        VerbatimDelta(days=-5, nanoseconds=0),
    ],
)
def test_pickle(d):
    dumped = pickle.dumps(d)
    assert len(dumped) < 100
    assert pickle.loads(dumped) == d


def test_compatible_unpickle():
    # This is a pickle of VerbatimDelta created with the initial implementation.
    # We keep this test to ensure backwards compatibility.
    dumped = (
        b"\x80\x04\x95A\x00\x00\x00\x00\x00\x00\x00\x8c\x14whenever._pywheneve"
        b"r\x94\x8c\r_unpkl_vdelta\x94\x93\x94(K\x01K\x01K\x02K\x03K\x04K\x05K\x06K"
        b"\x07K\x08t\x94R\x94."
    )
    result = pickle.loads(dumped)
    assert result == VerbatimDelta(
        years=1,
        months=2,
        weeks=3,
        days=4,
        hours=5,
        minutes=6,
        seconds=7,
        nanoseconds=8,
    )
