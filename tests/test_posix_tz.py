...
class TestParse:
    ...
    def test_fixed_offset(self, s, expected):
        assert parse(s) == Tz(expected, dst=None)
...
class TestCalculateOffsets:
    ...
    def test_calculate_offsets(self, tz, ymd, hms, expected):
        ...
        if isinstance(expected, Unambiguous):
            epoch = to_epoch_s(y, m, d, hour, minute, second, expected.offset)
            assert tz.offset_for_instant(epoch) == expected.offset
