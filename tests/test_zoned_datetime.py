...
def test_offset():
    d = ZonedDateTime(2020, 8, 15, 5, 12, 30, 450, tz="America/New_York")
...
class TestWithTime:
    def test_unambiguous(self):
        d = ZonedDateTime(2020, 8, 15, 14, tz="Europe/Amsterdam")
        assert d.with_time(Time(1, 2, 3, 4_000)) == ZonedDateTime(
            2020, 8, 15, 1, 2, 3, 4_000, tz="Europe/Amsterdam"
        )
    ...
    def test_ambiguous(self):
        ...
        assert d.with_time(time, disambiguate="earlier") == d.replace(
            hour=2, minute=15, second=30, disambiguate="earlier"
        )
        assert d.with_time(time, disambiguate="later") == d.replace(
            hour=2, minute=15, second=30, disambiguate="later"
        )
        assert d.with_time(time, disambiguate="compatible") == d.replace(
            hour=2, minute=15, second=30, disambiguate="compatible"
        )
...
class TestDefaultFormat:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # TODO: a historical tz with seconds in offset
        ],
    )
    def test_default_format(self, d: ZonedDateTime, expected: str):
...
class TestEquality:
    ...
    def test_ambiguity_between_different_timezones(self):
        ...
        b = a.in_tz("America/New_York")
        assert a.in_utc() == b.in_utc()  # sanity check
        assert hash(a) == hash(b)
        assert a == b
    ...
    def test_other_aware(self):
        ...
        assert d == d.in_utc()
        assert hash(d) == hash(d.in_utc())
        assert d != d.in_utc().replace(hour=23)

        assert d == d.in_local_system()
        assert d != d.in_local_system().replace(hour=8)

        assert d == d.in_fixed_offset()
        assert hash(d) == hash(d.in_fixed_offset())
        assert d != d.in_fixed_offset().replace(hour=10)
...
def test_in_utc():
    assert ZonedDateTime(
        2020, 8, 15, 12, 8, 30, tz="Europe/Amsterdam"
    ).in_utc() == UTCDateTime(2020, 8, 15, 10, 8, 30)
    d = ZonedDateTime(
        2023,
        10,
        29,
    ...
    assert d.in_utc() == UTCDateTime(2023, 10, 29, 0, 15, 30)
    assert ZonedDateTime(
        2023, 10, 29, 2, 15, 30, tz="Europe/Amsterdam", disambiguate="later"
    ).in_utc() == UTCDateTime(2023, 10, 29, 1, 15, 30)
...
def test_in_tz():
    ...
    assert nyc.in_tz("Europe/Amsterdam").exact_eq(ams)
    assert (
        nyc.replace(hour=21)
...
def test_in_fixed_offset():
    ...
    assert d.in_fixed_offset(-4).exact_eq(
...
class TestFromDefaultFormat:
    ...
    def test_valid(self, s, expect):
        assert ZonedDateTime.from_default_format(s).exact_eq(expect)
...
class TestTimestamp:
    ...
    def test_default_seconds(self):
        ...
        assert (
            ZonedDateTime(
                2020, 8, 15, 8, 8, 30, 45_123, tz="America/New_York"
            ).timestamp()
            == 1_597_493_310
        )
    ...
    def test_millis(self):
        ...
        assert (
            ZonedDateTime(
                2020, 8, 15, 8, 8, 30, 45_923_789, tz="America/New_York"
            ).timestamp_millis()
            == 1_597_493_310_045
        )
    ...
    def test_nanos(self):
        ...
        assert (
            ZonedDateTime(
                2020, 8, 15, 8, 8, 30, 45_123_789, tz="America/New_York"
            ).timestamp_nanos()
...
class TestFromTimestamp:
    ...
    def test_nanos(self):
        assert ZonedDateTime.from_timestamp_nanos(
            ...
            ZonedDateTime(
                2020, 8, 15, 10, 8, 30, 123_456_789, tz="America/Nuuk"
            )
        )
    ...
    def test_millis(self):
        assert ZonedDateTime.from_timestamp_millis(
            ...
            ZonedDateTime(
                2020, 8, 15, 10, 8, 30, 123_000_000, tz="America/Nuuk"
            )
        )


def test_repr():
    d = ZonedDateTime(2020, 8, 15, 23, 12, 9, 9_876_543, tz="Australia/Darwin")
    assert (
...
class TestComparison:
    ...
    def test_different_timezone_same_time(self):
        ...
        other = d.in_tz("America/New_York")
        assert not d < other
        assert d <= other
        assert not other > d
        assert other >= d
...
def test_py_datetime():
    d = ZonedDateTime(
        2020, 8, 15, 23, 12, 9, 987_654_999, tz="Europe/Amsterdam"
    )
...
def test_from_py_datetime():
    ...
    assert ZonedDateTime.from_py_datetime(d).exact_eq(
        ZonedDateTime(2020, 8, 15, 23, 12, 9, 987_654_000, tz="Europe/Paris")
    )
    ...
    with pytest.raises(
        SkippedTime,
        match="2023-03-26.*02:15:30.*skipped.*Europe/Amsterdam",
    ):
...
class TestAddDateUnits:
    ...
    def test_ambiguity(self):
        ...
        # TODO: what if combining durations first?
...
class TestSubtractDateUnits:
    ...
    def test_exceptions(self):
        with pytest.raises(ValueError, match="range"):
            ZonedDateTime(2020, 8, 15, tz="Europe/Amsterdam") - years(9000)
