...
#
# Run through the Makefile so dependencies and the release extension are ready:
#   make bench-compare
...
# followed by updated SVG charts in charts/.
#
...
OUT="results"
mkdir -p "$OUT"
rm -f "$OUT"/result_*.json

echo "==> Environment"
echo "    Python:   $(python --version)"
echo "    whenever: $(python -c 'import whenever; print(whenever.__version__)')"
echo "    arrow:    $(python -c 'import arrow; print(arrow.__version__)')"
echo "    pendulum: $(python -c 'from importlib.metadata import version; print(version("pendulum"))')"
...
run_bench() {
    ...
    local out_file="$2"
    echo "==> Running $script..."
    python "$script" $FAST_FLAG -o "$out_file"
}
...
python charts.py --results-dir "$OUT" --output "$CHART_OUT"
if [ "$UPDATE_DOCS" -eq 1 ]; then
    python perf_charts.py --python "$(command -v python)" --output "$CHART_OUT"
fi
