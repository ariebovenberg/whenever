...
pub fn date_from_unix_days(c: &mut Criterion) {
    c.bench_function("Date from ord", |b| {
        b.iter(|| black_box(UnixDays::new_unchecked(30179)).date())
    });
}
...
pub fn offset_for_local_datetime(c: &mut Criterion) {
    ...
    c.bench_function("offset for local", |b| {
        b.iter(|| tzif.ambiguity_for_local(black_box(EpochSecs::new_unchecked(1719946800))))
    });
}

pub fn tomorrow(c: &mut Criterion) {
    c.bench_function("Date::tomorrow()", |b| {
        b.iter(|| {
            let date = black_box(Date {
                year: Year::new_unchecked(2023),
                month: Month::new_unchecked(3),
                day: 2,
            });
