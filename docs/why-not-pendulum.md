...
myst:
  html_meta:
    description: >-
      A detailed comparison with Pendulum: why an API cannot be fixed by
      subclassing it, and how four further design decisions — treat naive
      datetimes as a mistake, redefine fold, put months inside timedelta, and
      complete the input rather than reject it — produce failures in equality,
      arithmetic, DST handling, and parsing that cannot be patched out.
---
...
After all: a subclass can't reliably guess which semantics the caller expects.
...
During a time zone conversion, the standard library's machinery
...
"wrong" branch, and the time zone is lost along the way:
...
It shipped its own time zone implementation, whose `fromutc()` never called `+`.
...
If a naive result is unacceptable, then Pendulum must invent a time zone
...
Even passing a time zone explicitly is inconsistent: given a `tz`, `instance()`
...
The `fold` flag (PEP 495) on `datetime` is the standard library's way to tell apart
the two occurrences of a repeated local time. Pendulum keeps the flag but gives it a meaning of its own:
...
which instant a repeated local time represents.
...
In a time zone where DST starts at midnight, midnight itself is skipped once a year.
...
within one time zone, `==` and `<` compare the local time and ignore `fold`.
...
Pendulum applies *all* units to the local clock, hours included:
...
units) computed
from the local clock fields. When the two ends share a time zone, that decomposition
...
| `Time` arithmetic drops the time zone and microseconds | [#362](https://github.com/python-pendulum/pendulum/issues/362), [#584](https://github.com/python-pendulum/pendulum/issues/584) | Apr 2019 | — |
...
Other tokens are recognized but not implemented, and come out as themselves:

```python
>>> d = pendulum.datetime(2024, 1, 15, 12)
>>> d.format("Today is dddd")
'To1ay i0 Monday'
>>> d.format("GGGG-[W]WW-E")      # ISO week date
'GGGG-WWW-1'
>>> d.format("h:mm a"), d.format("h:mm A")
('12:00 a', '12:00 PM')
```
...
Locale, week start, and the system time zone are process-wide settings, so the
...
And guessing (at the caller's intent, at missing time zones, at incomplete input)
