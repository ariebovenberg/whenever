...
myst:
  html_meta:
    description: >-
      Reference for ISO 8601 support: each type's canonical representation,
      parse_iso() and format_iso(), the basic format, and what is accepted when
      parsing.
---
...
for all date/time and delta types.
...
This makes the types round-trippable to text and suitable for lossless
serialization, except in the uncommon case of a {class}`ZonedDateTime` backed
by a system time zone without a time zone ID. Its time zone rules cannot be
reconstructed from local fields and a numeric offset alone; see
{ref}`systemtime`.
...
| {class}`IsoWeekDate`       | `2026-W04-5`                                       | {meth}`~IsoWeekDate.format_iso`       | {meth}`~IsoWeekDate.parse_iso`               |

[^1]: The time zone ID in square brackets is part of the recent RFC 9557 extension to RFC 3339.
      It may not be supported by other systems.

The `repr()` of each class is a constructor call that rebuilds an equal value.
For readability it writes a space between the date and the time, which the
constructor accepts, while `str()` writes the standard `T`; the delta
reprs use lowercase units for the same reason. It can even be used to
...
The basic format is accepted by all parsers,
and is written by the formatters that take `basic=`.
...
- Week and ordinal date formats are *not* supported by the {class}`Date` and datetime parsers:
  e.g. `2023-W52-5` or `2023-365`. Week dates can be parsed separately with {class}`IsoWeekDate`.
...
- ISO designator characters may be lowercase or uppercase (e.g. `2023-12-28T11:30:00Z`
  is the same as ``2023-12-28t11:30:00z``). Time zone IDs are looked up
  case-insensitively and are formatted with the spelling from the IANA time zone database.
  Aliases such as `US/Eastern` are preserved rather than replaced with the canonical time zone ID.
...
- The ``Z`` suffix, which denotes UTC, is accepted for {class}`OffsetDateTime`
  and treated as ``+00:00``.
- {meth}`Instant.parse_iso` requires ``Z`` or an offset, and converts a non-zero
  offset to UTC.
- Offsets may be specified up to second-level precision (e.g. `2023-12-28T11:30:00+01:23:45`).
- A time zone ID may be included in square brackets,
  like `2023-12-28T11:30:00+01[Europe/Paris]`. For {class}`ZonedDateTime`,
  the ID is required but the preceding offset is optional.
  This annotation is part of the recent RFC 9557 extension to RFC 3339.
- For {class}`ZonedDateTime`, an offset that disagrees with the time zone's
  rules raises {exc}`~whenever.InvalidOffsetError`. `parse_iso()` and the
  constructor take `offset_mismatch=` to resolve it instead, and
  `disambiguation=` for a local time without an offset; see
  {ref}`offset-mismatch`.
- {class}`Instant` and {class}`OffsetDateTime` accept a bracketed time zone ID
  and ignore it; only the offset is read. {class}`PlainDateTime` rejects a
  bracket, as it rejects an offset.
- ISO strings are ASCII. A string with other characters is rejected as an
  invalid ISO string before any part of it is read, so a non-ASCII time zone
  ID in a bracket raises {exc}`ValueError`, not
...
- `unit` controls the smallest unit to include, ranging from `"hour"` to `"nanosecond"`.
  ...
  >>> i.format_iso(unit="minute")
  '2025-09-28T21:24Z'
  >>> i.format_iso(unit="hour")  # the smaller fields are dropped
  '2025-09-28T21Z'
  >>> i.format_iso(unit="nanosecond")
...
- `tz_id_display` is supported by {meth}`ZonedDateTime.format_iso` and controls whether to
  include the time zone ID in square brackets.
  `"required"` (the default) raises {class}`ValueError` when the time zone has no ID,
  which happens for some system time zones. `"if_available"` writes the ID when
  there is one and omits it otherwise. `"omit"` never writes it.

  ```python
  >>> d = ZonedDateTime.now("Europe/Amsterdam")
  >>> d.format_iso(tz_id_display="if_available")
  '2025-09-28T23:24:17.664328+02:00[Europe/Amsterdam]'
  >>> d.format_iso(tz_id_display="omit")
