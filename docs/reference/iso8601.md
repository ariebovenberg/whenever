...
by a system timezone without a timezone ID. Its timezone rules cannot be
...
[^1]: The IANA timezone identifier in square brackets is part of the recent RFC 9557 extension to RFC 3339.
...
- ISO designator characters may be lowercase or uppercase (e.g. `2023-12-28T11:30:00Z`
  is the same as ``2023-12-28t11:30:00z``). IANA timezone identifiers are looked up
  case-insensitively and are formatted with the spelling from the timezone database.
...
- `tz_id_display` is supported by {meth}`ZonedDateTime.format_iso` and controls whether to
  include the timezone ID in square brackets.
  `"required"` (the default) raises {class}`ValueError` when the timezone has no ID,
  which happens for some system timezones. `"if_available"` writes the ID when
