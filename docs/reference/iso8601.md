...
[^1]: The IANA time zone identifier in square brackets is part of the recent RFC 9557 extension to RFC 3339.
...
constructor accepts, while `str()` writes the standard `T`; `TimeDelta`
...
- ISO designator characters may be lowercase or uppercase (e.g. `2023-12-28T11:30:00Z`
  is the same as ``2023-12-28t11:30:00z``). IANA time zone identifiers are looked up
  case-insensitively and are formatted with the spelling from the time zone database.
  Aliases such as `US/Eastern` are preserved rather than replaced with a primary zone.
...
- An IANA time zone identifier may be included in square brackets,
