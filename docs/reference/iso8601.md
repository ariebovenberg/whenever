...
by a system timezone without an IANA identifier. Its timezone rules cannot be
...
- `tz_id_display` is supported by {meth}`ZonedDateTime.format_iso` and controls whether to
  ...
  Default is `"required"` which will raise an error if there is no timezone identifier
  ...
  >>> d.format_iso(tz_id_display="auto")
  '2025-09-28T23:24:17.664328+02:00[Europe/Amsterdam]'
  >>> d.format_iso(tz_id_display="never")
