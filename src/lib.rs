...
use classes::{
    ...
    instant::{self, unpickle as _unpkl_inst, unpickle_v07 as _unpkl_utc, Instant},
...
unsafe fn new_enum(name: &CStr, members: &[(&CStr, i32)]) -> PyReturn {
...
unsafe extern "C" fn module_exec(module: *mut PyObject) -> c_int {
    ...
    if !new_type(
        ...
        c"_unpkl_date",
        date::SINGLETONS,
        &raw mut state.date_type,
        &raw mut state.unpickle_date,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.yearmonth_type,
        &raw mut state.unpickle_yearmonth,
    ) || !new_type(
        module,
        module_name,
        ...
        c"_unpkl_md",
        monthday::SINGLETONS,
        &raw mut state.monthday_type,
        &raw mut state.unpickle_monthday,
    ) || !new_type(
        module,
        module_name,
        &raw mut time::SPEC,
        c"_unpkl_time",
        time::SINGLETONS,
        &raw mut state.time_type,
        &raw mut state.unpickle_time,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.date_delta_type,
        &raw mut state.unpickle_date_delta,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.time_delta_type,
        &raw mut state.unpickle_time_delta,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.datetime_delta_type,
        &raw mut state.unpickle_datetime_delta,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.plain_datetime_type,
        &raw mut state.unpickle_plain_datetime,
    ) || !new_type(
        module,
        module_name,
        ...
        c"_unpkl_inst",
        instant::SINGLETONS,
        &raw mut state.instant_type,
        &raw mut state.unpickle_instant,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.offset_datetime_type,
        &raw mut state.unpickle_offset_datetime,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.zoned_datetime_type,
        &raw mut state.unpickle_zoned_datetime,
    ) || !new_type(
        module,
        module_name,
        ...
        &raw mut state.system_datetime_type,
        &raw mut state.unpickle_system_datetime,
    ...
    let datetime_cls = unwrap_or_errcode!(import_from(c"datetime", c"datetime"));
    defer_decref!(datetime_cls);
    state.strptime = PyObject_GetAttrString(datetime_cls, c"strptime".as_ptr());
    state.time_ns = unwrap_or_errcode!(import_from(c"time", c"time_ns"));

    let weekday_enum = unwrap_or_errcode!(new_enum(
        c"Weekday",
        &[
            (c"MONDAY", 1),
            (c"TUESDAY", 2),
    ...
    (&raw mut state.tz_store).write(TzStore::new(unwrap_or_errcode!(get_tzdata_path())));
