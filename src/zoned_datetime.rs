...
impl ZonedDateTime {
    ...
    pub(crate) unsafe fn resolve_using_disambiguate(
        ...
        let local_epoch = date.epoch_at(time);
    ...
    pub(crate) unsafe fn to_tz(self, tz: TzRef) -> Option<ZonedDateTime> {
        ...
        Some(
            epoch
                ...
                // We've already checked for both out-of-range date and time.
...
impl Display for ZonedDateTime {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "{}T{}{}[{}]", date, time, offset_fmt(offset), tz.key)
    }
}
...
unsafe fn py_datetime(slf: &mut PyObject, _: &mut PyObject) -> PyReturn {
    ...
    let DateTime {
        ...
        .without_offset()
        .change_offset(-zdt.offset.as_delta())
    ...
    // TODO: document that offsets could disagree
    methcall1(
        zoneinfo,
        "fromutc",
...
unsafe fn get_offset(slf: *mut PyObject) -> PyReturn {
    // TODO
    TimeDelta::from_secs_unchecked(ZonedDateTime::extract(slf).offset.get() as i64)
