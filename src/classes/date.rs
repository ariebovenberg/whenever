...
fn __sub__(obj_a: PyObj, obj_b: PyObj) -> PyReturn2 {
    ...
    // Easy case: Date - Date
    if type_b == type_a {
        let a: Date = obj_a.extract_unchecked();
        let b: Date = obj_b.extract_unchecked();
...
unsafe fn _shift_method(
    ...
    let (mut months, mut days) = match (args, kwargs.len()) {
        (&[arg], 0) => {
            ...
            if Py_TYPE(arg) == delta_type.as_ptr().cast() {
...
pub(crate) unsafe fn unpickle(module: *mut PyObject, arg: *mut PyObject) -> PyReturn {
    ...
    .to_obj(State::for_mod(module).date_type.as_ptr().cast())
}
