...
use pyo3_ffi::*;
use std::{ffi::CStr, fmt::Debug, mem::ManuallyDrop, ptr::null_mut as NULL};
...
impl PyObj {
    ...
    // TODO unchecked
    pub(crate) fn extract_unchecked<T: FromPy>(&self) -> T {
        unsafe { T::from_py_ptr_unchecked(self.inner.as_ptr()) }
    }

    // TODO name
    pub(crate) fn extract2<T: FromPy>(&self, t: PyType) -> Option<T> {
        (self.class() == t).then(|| self.extract_unchecked())
    }
...
impl PyType {
    ...
    pub(crate) fn same_module(&self, other: PyType) -> bool {
        // TODO: need to account for errors!
        unsafe { PyType_GetModule(self.as_ptr().cast()) == PyType_GetModule(other.as_ptr().cast()) }
    }
}
...
// impl PyTuple {
//     pub(crate) fn new<const N: usize>(objs: [PyObj; N]) -> PyResult<Owned<PyTuple>> {
//         Ok(unsafe { PyTuple_New(size) }.rust_owned()?.cast_unchecked())
//     }

//     pub(crate) fn get_item(&self, index: Py_ssize_t) -> PyResult<PyObj> {
//         let item = unsafe { PyTuple_GetItem(self.as_ptr(), index) };
//         if item.is_null() {
//             return Err(PyErrOccurred());
//         }
//         Ok(unsafe { PyObj::new_unchecked(item) })
//     }
// }
...
pub(crate) fn not_implemented() -> Owned<PyObj> {
    unsafe { Py_NotImplemented() }.rust_owned().unwrap()
}

#[allow(unused_imports)]
pub(crate) use {defer_decref, pack, parse_args_kwargs, steal, unpack_one};
