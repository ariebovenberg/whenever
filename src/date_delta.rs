...
pub(crate) unsafe extern "C" fn years(
    ...
    let parsed_amount = try_get_int!(amount);
...
unsafe extern "C" fn __mul__(slf: *mut PyObject, factor_obj: *mut PyObject) -> *mut PyObject {
    let factor = try_get_int!(factor_obj);
    if factor == 1 {
        Py_INCREF(slf);
        return slf;
    }
...
pub(crate) unsafe extern "C" fn unpickle(
    ...
    new_unchecked(
        (*PyModule_GetState(module).cast::<crate::ModuleState>()).date_delta_type,
        DateDelta {
            years: try_get_int!(*args.offset(0)) as i16,
            months: try_get_int!(*args.offset(1)) as i32,
            weeks: try_get_int!(*args.offset(2)) as i32,
            days: try_get_int!(*args.offset(3)) as i32,
        },
    )
    .cast()
}
