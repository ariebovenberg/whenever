...
impl DateDelta {
    ...
    pub fn new(months: DeltaMonths, days: DeltaDays) -> Option<Self> {
        same_sign(months, days).then_some(Self { months, days })
    }

    pub fn from_months(months: DeltaMonths) -> Self {
        Self {
            months,
            days: DeltaDays::ZERO,
        }
    ...
    pub fn from_days(days: DeltaDays) -> Self {
        Self {
    ...
    pub fn checked_add(self, other: Self) -> Result<Self, InitError> {
        let Self { months, days } = self;
...
pub const SINGLETONS: &[(&CStr, DateDelta); 1] = &[(c"ZERO", DateDelta::ZERO)];
...
pub fn format_components(delta: DateDelta, s: &mut String) {
...
pub unsafe fn handle_init_kwargs<T>(
    fname: &str,
    kwargs: T,
...
pub unsafe fn years(module: *mut PyObject, amount: *mut PyObject) -> PyReturn {
    amount
        .to_long()?
...
pub unsafe fn months(module: *mut PyObject, amount: *mut PyObject) -> PyReturn {
...
pub unsafe fn weeks(module: *mut PyObject, amount: *mut PyObject) -> PyReturn {
    amount
        .to_long()?
        .ok_or_type_err("argument must be int")?
        .checked_mul(7)
...
pub unsafe fn days(module: *mut PyObject, amount: *mut PyObject) -> PyReturn {
...
pub fn parse_prefix(s: &mut &[u8]) -> Option<bool> {
...
pub fn parse_component(s: &mut &[u8]) -> Option<(i32, Unit)> {
...
pub unsafe fn unpickle(module: *mut PyObject, args: &[*mut PyObject]) -> PyReturn {
    if args.len() == 2 {
        DateDelta {
...
pub static mut SPEC: PyType_Spec = type_spec::<DateDelta>(c"whenever.DateDelta", unsafe { SLOTS });
