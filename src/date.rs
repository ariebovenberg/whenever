...
pub const SINGLETONS: &[(&CStr, Date); 2] = &[(c"MIN", Date::MIN), (c"MAX", Date::MAX)];
...
impl Date {
    ...
    pub fn last_of_month(year: Year, month: Month) -> Self {
        Date {
            year,
            month,
    ...
    pub fn first_of_month(year: Year, month: Month) -> Self {
        Date {
            year,
            month,
            day: 1,
    ...
    pub fn from_longs(y: c_long, m: c_long, day: c_long) -> Option<Self> {
    ...
    pub const fn unix_days(self) -> UnixDays {
    ...
    pub fn epoch_at(self, t: Time) -> EpochSecs {
        self.unix_days()
            .epoch()
            // Safety: unix days are always in range until midnight
            .add_unchecked(t.total_seconds())
    }

    pub fn epoch(self) -> EpochSecs {
        EpochSecs::new_unchecked(self.unix_days().get() as i64 * S_PER_DAY as i64)
    }

    pub fn shift(self, months: DeltaMonths, days: DeltaDays) -> Option<Date> {
        self.shift_months(months).and_then(|x| x.shift_days(days))
    }

    pub fn shift_days(self, days: DeltaDays) -> Option<Date> {
        Some(self.unix_days().shift(days)?.date())
    }

    pub fn shift_months(self, months: DeltaMonths) -> Option<Date> {
    ...
    pub fn parse_all(s: &[u8]) -> Option<Self> {
    ...
    pub fn parse_partial(s: &mut &[u8]) -> Option<Self> {
    ...
    pub unsafe fn from_py_unchecked(obj: *mut PyObject) -> Self {
    ...
    pub const unsafe fn hash(self) -> i32 {
...
pub const MAX_YEAR: c_long = 9999;
...
pub const fn days_before_year(year: Year) -> i32 {
...
pub static mut SPEC: PyType_Spec = type_spec::<Date>(c"whenever.Date", unsafe { SLOTS });
