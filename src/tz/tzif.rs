        ...
        bisect_index(&self.offsets_by_local, t)
            .map(|i| {
                ...
                match ambiguity_duration.get().cmp(&0) {
                    ...
                    // The shifts here are unchecked since they were
                    // calculated from the offsets themselves.
                    Less => Ambiguity::Fold(offset.shift_unchecked(-ambiguity_duration), offset),
                    Greater => Ambiguity::Gap(offset, offset.shift_unchecked(-ambiguity_duration)),
                }
            })
...
#[inline]
pub fn bisect_index<T>(arr: &[(EpochSecs, T)], x: EpochSecs) -> Option<usize>
where
    T: Copy,
{
...
fn parse_transition_times(header: Header, s: &mut Scan) -> Option<Vec<EpochSecs>> {
    ...
    for i in 0..header.timecnt {
        // TODO: test very lange time values
