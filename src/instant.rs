...
pub const SINGLETONS: &[(&CStr, Instant); 2] = &[
    (
        c"MIN",
        Instant {
            epoch: EpochSecs::MIN,
...
impl Instant {
    pub fn from_datetime(date: Date, time: Time) -> Self {
        Instant {
    ...
    pub fn from_timestamp_f64(timestamp: f64) -> Option<Self> {
    ...
    pub fn from_timestamp_nanos(timestamp: i128) -> Option<Self> {
    ...
    pub fn shift(&self, d: TimeDelta) -> Option<Instant> {
        let (extra_sec, subsec) = self.subsec.add(d.subsec);
        Some(Instant {
            epoch: self.epoch.add_i64(d.secs.get() + extra_sec.get())?,
            subsec,
        })
    }

    pub fn offset(&self, f: Offset) -> Option<Self> {
        Some(Instant {
...
pub static mut SPEC: PyType_Spec = type_spec::<Instant>(c"whenever.Instant", unsafe { SLOTS });
